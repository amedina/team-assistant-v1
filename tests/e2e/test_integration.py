...
class TestFullPipeline:
    ...
    def test_end_to_end_document_flow(self, system_config, text_processor, test_results: E2ETestResults):
        ...
        async def run_test():
            try:
                ...
                return True, f"Pipeline completed: {len(processed_doc.chunks)} chunks, {vector_result.successful_count} vector, {db_result.successful_count} db"
        ...
        test_results.add_result("end_to_end_document_flow", success, details, execution_time)
        if not success:
...
class TestContextManager:
    ...
    def test_context_manager_four_step_flow(self, system_config, test_results: E2ETestResults):
        ...
        async def run_test():
            try:
                ...
                if not vector_results:
                    ...
                    return True, "Context manager flow validated (no existing data to query)"
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                return True, f"4-step flow completed: {structured_context['vector_results']['count']} vector, {structured_context['database_metadata']['count']} db, {structured_context['knowledge_graph_context']['entity_count']} entities"
        ...
        test_results.add_result("context_manager_flow", success, details, execution_time)
        if not success:
...
class TestSystemHealth:
    ...
    def test_comprehensive_system_health(self, system_config, test_results: E2ETestResults):
        ...
        async def run_test():
            try:
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                return True, f"System health: {component_health}"
        ...
        test_results.add_result("comprehensive_system_health", success, details, execution_time)
        if not success:
