...
def demonstrate_refined_secret_resolution():
    ...
    try:
        ...
        print(f"   ✅ Resolved neo4j_password: {'*' * len(neo4j_password)}")
    ...
    try:
        ...
        print(f"   ✅ Resolved db_pass: {'*' * len(db_pass)}")
        print("   🔒 Resolution order:")
    ...
    try:
        ...
        if system_config.pipeline_config.neo4j:
            ...
            print(f"   Neo4j Password: {'*' * len(neo4j_password)} (from secure source)")
        ...
        if system_config.pipeline_config.database:
            ...
            print(f"   Database Pass: {'*' * len(db_pass)} (from secure source)")
...
def demonstrate_environment_override():
    ...
    try:
        ...
        try:
            neo4j_password = config_manager.resolve_secret('neo4j_password')
            source = "Secret Manager" if len(neo4j_password) > 20 else "Environment Variable"
        ...
        try:
            db_pass = config_manager.resolve_secret('db_pass')
            source = "Secret Manager" if len(db_pass) > 20 else "Environment Variable"
